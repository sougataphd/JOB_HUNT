"""Thin Apify client for the JOB_HUNT LinkedIn skill.

Read-only. Apify runs public, no-cookies scraper actors — it cannot post,
comment, or otherwise write to LinkedIn on your behalf. Every method wraps
one actor via the run-sync-get-dataset-items endpoint, so the caller gets
results back in a single HTTP request (no polling required).
...
Actors used (all no-cookies, public, "$1-$5 per 1,000 results"):
  ...
  - apimaestro/linkedin-post-comments-replies-engagements-scraper-no-cookies
      Fetch comments + replies on a post (by post ID or URL).
  - apimaestro/linkedin-profile-comments
      Fetch a user's recent comments by username.
  - scraping_solutions/linkedin-posts-engagers-likers-and-commenters-no-cookies
      Fetch the people who liked or commented on a post.
  - apimaestro/linkedin-profile-detail
      Fetch a profile's headline, about, experience, education, skills by
      username (the `in/<username>` slug). Used by li-profile to pull your
      own profile before scoring/rewriting it.
...
Adapted from https://github.com/sergebulaev/linkedin-skills (MIT), with a
`fetch_profile` method added for job-search profile auditing.
"""
...
class ApifyClient:
    ...
    def fetch_profile(
        ...
        *,
        ...
        """Return a profile's headline, about, experience, education, skills.

        Args:
            profile_url_or_username: full profile URL
                (https://www.linkedin.com/in/<username>) or bare `<username>`.
            force_refresh: bypass cache.

        Use for auditing your own profile in li-profile, or a target
        researcher/contact's public profile before drafting an outreach note.
        Only public profile data is returned; no login or cookies used.
        """
        username = profile_url_or_username.rstrip("/").split("/in/")[-1]
        items = self._run_sync(
        ...
        if not items:
            raise ApifyError(f"no profile returned for {profile_url_or_username}")
        return items[0]
